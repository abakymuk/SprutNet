...
Инструкции по настройке и управлению базой данных Supabase для проекта SprutNet с интеграцией Maersk API.
...
- [Maersk API интеграция](#maersk-api-интеграция)
...
## 🚢 Maersk API интеграция

База данных включает полную нормализованную модель для работы с Maersk API:

### Основные таблицы
- **`vessels`** - информация о судах (IMO номер, название, вместимость)
- **`locations`** - порты, города, страны (Geo ID, UN/LOCODE)
- **`point_to_point_schedules`** - расписания маршрутов
- **`transport_legs`** - отдельные участки маршрута
- **`deadlines`** - дедлайны отправки
- **`facilities`** - терминалы и объекты

### Справочные таблицы
- **`countries`** - страны (ISO 3166-1)
- **`carrier_codes`** - коды перевозчиков (SCAC)
- **`transport_modes`** - режимы транспорта
- **`location_types`** - типы локаций

### Миграция
```bash
# Применить миграцию Maersk модели
supabase db reset
```

Подробная документация: [ERD.md](../docs/ERD.md)
